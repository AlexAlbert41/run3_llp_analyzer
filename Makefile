...
all: copy_runners copy_runners_TnP copy_runners_TnP_noClusters copy_runners_TnP_mdsnano copy_runners_TrigEff copy_runners_TrigEff_mdsnano $(EXECUTABLES)
...
copy_runners:
		@for d in $(subst Run,,$(notdir $(basename $(RUNNERSCC)))); do \
			if [ $$d != "llp_MuonSystem_CA_TnP" ] && [ $$d != "llp_MuonSystem_CA_TrigEff" ] && [ $$d != "llp_MuonSystem_CA_TrigEff_mdsnano" ]&& [ $$d != "llp_MuonSystem_CA_TnP_noClusters" ] && [ $$d != "llp_MuonSystem_CA_TnP_mdsnano" ]; then \
