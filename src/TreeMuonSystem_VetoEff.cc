...
#include "TreeMuonSystem_VetoEff.h"
#include "assert.h"
#include "TTree.h"
#include "DBSCAN.h"

// Constructor
TreeMuonSystem_VetoEff::TreeMuonSystem_VetoEff()
{
  InitVariables();
};
TreeMuonSystem_VetoEff::~TreeMuonSystem_VetoEff()
...
void TreeMuonSystem_VetoEff::InitVariables()
{
  ...
  //added for veto eff measurement
  sampledEta = 0.; sampledPhi = 0.; sampledCSC = false; sampledDT = false; numMatchedJets_CSC = 0; numMatchedJets_DT = 0; 
  numMatchedRechits_CSC = 0; numMatchedRechits_DT = 0; numMatchedMuons_CSC = 0; numMatchedMuons_DT_lowMET = 0; numMatchedMuons_DT_highMET = 0;
...
void TreeMuonSystem_VetoEff::InitTree()
{
  assert(tree_);
  InitVariables();

  //added for veto eff measurement
  tree_->SetBranchAddress("sampledEta",      &sampledEta);
  tree_->SetBranchAddress("sampledPhi",      &sampledPhi);
  tree_->SetBranchAddress("sampledCSC",      &sampledCSC);
  tree_->SetBranchAddress("sampledDT",      &sampledDT);
  tree_->SetBranchAddress("numMatchedJets_CSC",      &numMatchedJets_CSC);
  tree_->SetBranchAddress("numMatchedMuons_CSC",      &numMatchedMuons_CSC);
  tree_->SetBranchAddress("numMatchedRechits_CSC",      &numMatchedRechits_CSC);
  tree_->SetBranchAddress("numMatchedJets_DT",      &numMatchedJets_DT);
  tree_->SetBranchAddress("numMatchedMuons_DT_lowMET",      &numMatchedMuons_DT_lowMET);
  tree_->SetBranchAddress("numMatchedMuons_DT_highMET",      &numMatchedMuons_DT_highMET);
  tree_->SetBranchAddress("numMatchedRechits_DT",      &numMatchedRechits_DT);

  tree_->SetBranchAddress("matchedJetPt_CSC",      matchedJetPt_CSC);
  tree_->SetBranchAddress("matchedJetEta_CSC",      matchedJetEta_CSC);
  tree_->SetBranchAddress("matchedJetPhi_CSC",      matchedJetPhi_CSC);
  tree_->SetBranchAddress("matchedJetPt_DT",      matchedJetPt_DT);
  tree_->SetBranchAddress("matchedJetEta_DT",      matchedJetEta_DT);
  tree_->SetBranchAddress("matchedJetPhi_DT",      matchedJetPhi_DT);
  tree_->SetBranchAddress("matchedMuonPt_CSC",      matchedMuonPt_CSC);
  tree_->SetBranchAddress("matchedMuonEta_CSC",      matchedMuonEta_CSC);
  tree_->SetBranchAddress("matchedMuonPhi_CSC",      matchedMuonPhi_CSC);
  tree_->SetBranchAddress("matchedMuonPt_DT_lowMET",      matchedMuonPt_DT_lowMET);
  tree_->SetBranchAddress("matchedMuonEta_DT_lowMET",      matchedMuonEta_DT_lowMET);
  tree_->SetBranchAddress("matchedMuonPhi_DT_lowMET",      matchedMuonPhi_DT_lowMET);
  tree_->SetBranchAddress("matchedMuonPt_DT_highMET",      matchedMuonPt_DT_highMET);
  tree_->SetBranchAddress("matchedMuonEta_DT_highMET",      matchedMuonEta_DT_highMET);
  tree_->SetBranchAddress("matchedMuonPhi_DT_highMET",      matchedMuonPhi_DT_highMET);
...
void TreeMuonSystem_VetoEff::LoadTree(const char* file)
{
  f_ = TFile::Open(file);
  assert(f_);
...
void TreeMuonSystem_VetoEff::CreateTree()
{
  tree_ = new TTree("MuonSystem","MuonSystem");
  f_ = 0;

  tree_->Branch("sampledEta",      &sampledEta, "sampledEta/F");
  tree_->Branch("sampledPhi",      &sampledPhi, "sampledPhi/F");
  tree_->Branch("sampledCSC",      &sampledCSC, "sampledCSC/O");
  tree_->Branch("sampledDT",      &sampledDT, "sampledDT/O");
  tree_->Branch("numMatchedJets_CSC",      &numMatchedJets_CSC,     "numMatchedJets_CSC/I");
  tree_->Branch("numMatchedJets_DT",      &numMatchedJets_DT,     "numMatchedJets_DT/I");
  tree_->Branch("numMatchedMuons_CSC",      &numMatchedMuons_CSC,    "numMatchedMuons_CSC/I");
  tree_->Branch("numMatchedMuons_DT_lowMET",      &numMatchedMuons_DT_lowMET,    "numMatchedMuons_DT_lowMET/I");
  tree_->Branch("numMatchedMuons_DT_highMET",      &numMatchedMuons_DT_highMET,    "numMatchedMuons_DT_highMET/I");
  tree_->Branch("numMatchedRechits_CSC",      &numMatchedRechits_CSC,    "numMatchedRechits_CSC/I");
  tree_->Branch("numMatchedRechits_DT",      &numMatchedRechits_DT,    "numMatchedRechits_DT/I");

  tree_->Branch("matchedJetPt_CSC",      matchedJetPt_CSC,     "matchedJetPt_CSC[numMatchedJets_CSC]/F");
  tree_->Branch("matchedJetEta_CSC",      matchedJetEta_CSC,     "matchedJetEta_CSC[numMatchedJets_CSC]/F");
  tree_->Branch("matchedJetPhi_CSC",      matchedJetPhi_CSC,     "matchedJetPhi_CSC[numMatchedJets_CSC]/F");
  tree_->Branch("matchedJetPt_DT",      matchedJetPt_DT,     "matchedJetPt_DT[numMatchedJets_DT]/F");
  tree_->Branch("matchedJetEta_DT",      matchedJetEta_DT,     "matchedJetEta_DT[numMatchedJets_DT]/F");
  tree_->Branch("matchedJetPhi_DT",      matchedJetPhi_DT,     "matchedJetPhi_DT[numMatchedJets_DT]/F");
  
  
  tree_->Branch("matchedMuonPt_CSC",      matchedMuonPt_CSC,    "matchedMuonPt_CSC[numMatchedMuons_CSC]/F");
  tree_->Branch("matchedMuonEta_CSC",      matchedMuonEta_CSC,    "matchedMuonEta_CSC[numMatchedMuons_CSC]/F");
  tree_->Branch("matchedMuonPhi_CSC",      matchedMuonPhi_CSC,   "matchedMuonPhi_CSC[numMatchedMuons_CSC]/F");
  
  tree_->Branch("matchedMuonPt_DT_lowMET",      matchedMuonPt_DT_lowMET,    "matchedMuonPt_DT_lowMET[numMatchedMuons_DT_lowMET]/F");
  tree_->Branch("matchedMuonEta_DT_lowMET",      matchedMuonEta_DT_lowMET,   "matchedMuonEta_DT_lowMET[numMatchedMuons_DT_lowMET]/F");
  tree_->Branch("matchedMuonPhi_DT_lowMET",      matchedMuonPhi_DT_lowMET,  "matchedMuonPhi_DT_lowMET[numMatchedMuons_DT_lowMET]/F");
  tree_->Branch("matchedMuonPt_DT_highMET",      matchedMuonPt_DT_highMET,    "matchedMuonPt_DT_highMET[numMatchedMuons_DT_highMET]/F");
  tree_->Branch("matchedMuonEta_DT_highMET",      matchedMuonEta_DT_highMET,   "matchedMuonEta_DT_highMET[numMatchedMuons_DT_highMET]/F");
  tree_->Branch("matchedMuonPhi_DT_highMET",      matchedMuonPhi_DT_highMET,  "matchedMuonPhi_DT_highMET[numMatchedMuons_DT_highMET]/F");
