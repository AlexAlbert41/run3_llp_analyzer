...
Run <br> make <br>. If you get errors about something called "Fastjet", let me know :)
...
The <br> Merged_Cache_InputLists <br> directory houses the txt files that list the input MDSNANO files by era. I recommend making your own txt files with just a few input files that you can run over locally - you don't want to run over a whole era without condor.
...
You can check the script to see exactly what the different command line arguments do. The output path is partially hardcoded to my directory within the shared eos area, so I would fix that before running. One subtlety is that condor is only available on el8, so even though <br> make <br> should be ran in el7 along with the local tests, the condor command needs to happen in el8. The os of the condor machines is specified in the script, so if we upgrade everything to work with a newer os, this will need to be modified.
