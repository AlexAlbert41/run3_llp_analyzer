...
#include "llp_MuonSystem_CA_TnP_noClusters_VetoEff.h"
...
void llp_MuonSystem_CA_TnP_noClusters_VetoEff::Analyze(bool isData, int options, string outputfilename, string analysisTag)
{
...
//    cout<<"event num: "<<event<<endl;
      ...
      */
      ...
      for( int i = 0; i < nMuon; i++ )  
      {
        ...
        */
        ...
        if (RazorAnalyzer_trigEff::deltaR(sampled_eta, sampled_phi, Muon_eta[i], Muon_phi[i]) > 0.4) continue;
      ...
      if (RazorAnalyzer_trigEff::deltaR(Leptons[0].lepton.Eta(), Leptons[0].lepton.Phi(), sampled_eta, sampled_phi) < 0.4 || 
          RazorAnalyzer_trigEff::deltaR(Leptons[1].lepton.Eta(), Leptons[1].lepton.Phi(), sampled_eta, sampled_phi) < 0.4){
        continue;
      }
  ...
  for ( int i = 0; i < nJet; i++ )
  {
    if (RazorAnalyzer_trigEff::deltaR(sampled_eta, sampled_eta, Jet_eta[i], Jet_phi[i]) > 0.4) continue;
  ...
  if (matchedCSC){
    for (int i=0; i<nCscRechits;i++){
      ...
      if (RazorAnalyzer_trigEff::deltaR(sampled_eta, sampled_phi, cscRechitsEta[i], cscRechitsPhi[i]) < 0.4){
        matchedCSCHits++;
      }
    }
  }
  if (matchedDT){
    for (int i=0; i<nDtRechits;i++){
      ...
      if (RazorAnalyzer_trigEff::deltaR(sampled_eta, sampled_phi, dtRechitCorrectEta[i], dtRechitCorrectPhi[i]) < 0.4){
        matchedDTHits++;
      }
    }
  }
