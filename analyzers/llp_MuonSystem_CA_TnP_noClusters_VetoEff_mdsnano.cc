...
#include "llp_MuonSystem_CA_TnP_noClusters_VetoEff_mdsnano.h"
...
void llp_MuonSystem_CA_TnP_noClusters_VetoEff_mdsnano::Analyze(bool isData, int options, string outputfilename, string analysisTag)
{
  ...
  TreeMuonSystem_VetoEff *MuonSystem = new TreeMuonSystem_VetoEff();
  ...
  default_random_engine generatorEta;
  uniform_real_distribution<float> eta_distribution(-2.4, 2.4);
  default_random_engine generatorPhi;
  uniform_real_distribution<float> phi_distribution(-3.14, 3.14);
  float dump = phi_distribution(generatorPhi);
...
//    cout<<"event num: "<<event<<endl;
    ...
    */
      ...
      //generate random eta/phi point
     
      double sampled_eta = eta_distribution(generatorEta);
      double sampled_phi = phi_distribution(generatorPhi);

      MuonSystem->sampledEta = sampled_eta;
      MuonSystem->sampledPhi = sampled_phi;
      //cout<<"sampled eta: "<<sampled_eta<<endl;
      //cout<<"sampled phi: "<<sampled_phi<<endl;
      bool matchedCSC = false;
      bool matchedDT = false;

      if (abs(sampled_eta)<1.1){
        matchedDT = true;
      }
      if(abs(sampled_eta)>0.9){
        matchedCSC = true;
      }
      MuonSystem->sampledCSC = matchedCSC;
      MuonSystem->sampledDT = matchedDT;
      //cout<<"past sampling and initial loading"<<endl;
      /*
      ...
      */
      ...
      int nCSCMuons=0;
      int nDTMuons_lowMET=0;
      int nDTMuons_highMET=0;
      ...
      //cout<<"starting iteration through muons"<<endl;
      for( int i = 0; i < nMuon; i++ )  
      {
        ...
        */
        ...
        if (pass_looseId_muon && pass_eta_muon && pass_pt_muon && pass_looseIso_muon){
        Leptons.push_back(tmpMuon);
        continue;
        }
        //cout<<"checking for muon matches"<<endl;
        if (RazorAnalyzerMerged::deltaR(sampled_eta, sampled_phi, Muon_eta[i], Muon_phi[i]) > 0.4) continue;
        if (matchedCSC && Muon_isGlobal[i] && Muon_pt[i]>30 && pass_eta_muon){
          MuonSystem->matchedMuonPt_CSC[nCSCMuons] = Muon_pt[i];
          MuonSystem->matchedMuonEta_CSC[nCSCMuons] = Muon_eta[i];
          MuonSystem->matchedMuonPhi_CSC[nCSCMuons] = Muon_phi[i];
          nCSCMuons++;
        }
        if (matchedDT && Muon_looseId[i] && Muon_pt[i]>30 && pass_eta_muon){
          MuonSystem->matchedMuonPt_DT_lowMET[nDTMuons_lowMET] = Muon_pt[i];
          MuonSystem->matchedMuonEta_DT_lowMET[nDTMuons_lowMET] = Muon_eta[i];
          MuonSystem->matchedMuonPhi_DT_lowMET[nDTMuons_lowMET] = Muon_phi[i];
          nDTMuons_highMET++;
        }
        if (matchedDT && Muon_looseId[i] && Muon_pt[i]>10 && pass_eta_muon){
          MuonSystem->matchedMuonPt_DT_highMET[nDTMuons_highMET] = Muon_pt[i];
          MuonSystem->matchedMuonEta_DT_highMET[nDTMuons_highMET] = Muon_eta[i];
          MuonSystem->matchedMuonPhi_DT_highMET[nDTMuons_highMET] = Muon_phi[i];
          nDTMuons_lowMET++;
        }
        
      }
      if (matchedCSC){
        MuonSystem->numMatchedMuons_CSC = nCSCMuons;
      }
      if (matchedDT){
        MuonSystem->numMatchedMuons_DT_lowMET = nDTMuons_lowMET;
        MuonSystem->numMatchedMuons_DT_highMET = nDTMuons_highMET;
      }
      if (Leptons.size()!=2)continue;
      //check if TnP Muons are within a delta of 0.4 of sampled spot
      //cout<<"checking for TnP muon matches"<<endl;
      if (RazorAnalyzerMerged::deltaR(Leptons[0].lepton.Eta(), Leptons[0].lepton.Phi(), sampled_eta, sampled_phi) < 0.4 || 
          RazorAnalyzerMerged::deltaR(Leptons[1].lepton.Eta(), Leptons[1].lepton.Phi(), sampled_eta, sampled_phi) < 0.4){
        continue;
      }
      //cout<<"finished checking TnP matches"<<endl;
      if (Leptons.size() != 2) continue;
  ...
  //check for jet veto
  //cout<<"checking for jet veto"<<endl;
  int nMatchedJets_CSC=0;
  int nMatchedJets_DT=0;
  for ( int i = 0; i < nJet; i++ )
  {
    if (RazorAnalyzerMerged::deltaR(sampled_eta, sampled_eta, Jet_eta[i], Jet_phi[i]) > 0.4) continue;
    if (matchedCSC && Jet_pt[i]>30 && fabs(Jet_eta[i])<2.5){
      MuonSystem->matchedJetPt_CSC[nMatchedJets_CSC] = Jet_pt[i];
      MuonSystem->matchedJetEta_CSC[nMatchedJets_CSC] = Jet_eta[i];
      MuonSystem->matchedJetPhi_CSC[nMatchedJets_CSC] = Jet_phi[i];
      nMatchedJets_CSC++;
    }
    if (matchedDT && Jet_pt[i]>50 && fabs(Jet_eta[i])<2.5){
      MuonSystem->matchedJetPt_DT[nMatchedJets_DT] = Jet_pt[i];
      MuonSystem->matchedJetEta_DT[nMatchedJets_DT] = Jet_eta[i];
      MuonSystem->matchedJetPhi_DT[nMatchedJets_DT] = Jet_phi[i];
      nMatchedJets_DT++;
    }

  }
  MuonSystem->numMatchedJets_CSC = nMatchedJets_CSC;
  MuonSystem->numMatchedJets_DT = nMatchedJets_DT;


  //now check rechit vetoes
  //cout<<"checking for rechit veto"<<endl;
  int matchedCSCHits=0;int matchedDTHits=0;
  if (matchedCSC){
    for (int i=0; i<nCscRechits;i++){
      if (!(cscRechitsChamber[i]==11||cscRechitsChamber[i]==12||cscRechitsChamber[i]==-11||cscRechitsChamber[i]==-12)) continue;
      if (RazorAnalyzerMerged::deltaR(sampled_eta, sampled_phi, cscRechitsEta[i], cscRechitsPhi[i]) < 0.4){
        matchedCSCHits++;
      }
    }
  }
  if (matchedDT){
    for (int i=0; i<nDtRechits;i++){
      if (!(dtRechitStation[i]==1))continue;
      if (RazorAnalyzerMerged::deltaR(sampled_eta, sampled_phi, dtRechitCorrectEta[i], dtRechitCorrectPhi[i]) < 0.4){
        matchedDTHits++;
      }
    }
  }

  MuonSystem->numMatchedRechits_CSC = matchedCSCHits;
  MuonSystem->numMatchedRechits_DT = matchedDTHits;
